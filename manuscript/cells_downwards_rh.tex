...
\section{Symmetry Reduction of the Quadratic Form}

Recall the involution
\[
(Jg)(t) = g(-t),
\]
with decomposition
\[
g = g_+ + g_-, \qquad g_\pm(-t) = \pm g_\pm(t).
\]

\subsection{Even–Odd Splitting}

Assuming the zero set is symmetric under $\rho \mapsto 1-\rho$,
the quadratic form $Q$ is invariant under $J$.
...
\subsection{Reduction of RH}

The Riemann Hypothesis is equivalent to:

\[
Q(g_-) \ge 0
\quad \text{for all admissible odd } g.
\]

Thus the problem reduces to proving coercivity on the odd sector.

\end{document}

\section{Finite Truncation Test Model}

Fix an odd Schwartz basis $\{\psi_j\}_{j=1}^m \subset \mathcal{S}_-$ and write
\[
g_-(t)=\sum_{j=1}^m c_j \psi_j(t),\qquad c\in\mathbb{R}^m.
\]
Define the truncated quadratic form
\[
Q_m(c):=Q\!\left(\sum_{j=1}^m c_j \psi_j\right).
\]
The empirical coercivity test is:
\[
\lambda_{\min}(Q_m)\;>\;0.
\]
